...
module.exports = {
  ...
  async create(request, response) {
    ...
    const [id] = await connecion('incidents').insert({
      title,
      description,
      value,
      ong_id,
  ...
  async delete(request, response) {
    ...
    const incident = await connecion('incidents')
      .where('id', id)
      .select('ong_id')
      .first()
    ...
    await connecion('incidents').where('id', id).delete();

    return response.status(204).send([ "SUCESS" ])
